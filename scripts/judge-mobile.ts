...
requirePattern(scene, /COMPACT_VIEWPORT_QUERY = '\(max-width: 720px\), \(pointer: coarse\) and \(max-width: 900px\), \(pointer: coarse\) and \(max-height: 600px\)'/, 'Short coarse-pointer landscape screens are misclassified as desktop WebGL clients.')
...
requirePattern(mobile, /@media \(pointer: coarse\) and \(max-height: 600px\)[\s\S]*\.hero__viewport[\s\S]*min-height:\s*0/, 'Short landscape phones retain the desktop 48rem sticky viewport floor.')
requirePattern(hero, /COARSE_SMALL_VIEWPORT_QUERY = '\(pointer: coarse\) and \(max-width: 900px\), \(pointer: coarse\) and \(max-height: 600px\)'[\s\S]*static-compact-coarse/, 'Landscape phones wider than the narrow CSS breakpoint can still mount WebGL.')
