...
requirePattern(researchSection, /useScroll[\s\S]*ResearchVisual[\s\S]*taxonomyState[\s\S]*benchmarkState[\s\S]*deltaState/, 'Research is still a repeated editorial list instead of a scroll-led personal observatory.')
requirePattern(researchSection, /progress=\{scrollYProgress\}/, 'The research observatory is not driven by continuous scroll input.')
requirePattern(researchSection, /Conceptual map · schematic, not to scale/, 'The research visualization does not disclose that its topology is schematic.')
requirePattern(researchSection, /mobileExperience[\s\S]*research\.map[\s\S]*ResearchVisual/, 'The research observatory has no static small-screen fallback.')
