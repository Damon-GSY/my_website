...
const workSectionStyles = readFileSync(resolve(root, 'components/work-section-layout.css'), 'utf8')
...
requirePattern(workSectionStyles, /\.signature-story__panel\s*\{[^}]*min-height:\s*min\(52rem,\s*calc\(100dvh - 8\.2rem\)\)/s, 'Signature-case panels can stretch indefinitely on tall portrait viewports.')
requirePattern(workSectionStyles, /\.signature-story__panel--route \.case-signal\s*\{[^}]*height:\s*min\(42rem,\s*calc\(100dvh - 13rem\)\)[^}]*min-height:\s*32rem/s, 'The authority-routing visual lacks a bounded desktop height contract.')
requirePattern(workSectionStyles, /@media \(max-width: 1024px\)[\s\S]*\.signature-story__panel\s*\{[^}]*min-height:\s*min\(52rem,\s*calc\(100dvh - 7rem\)\)/s, 'Tablet signature-case panels can override the tall-viewport height cap.')
requirePattern(workSectionStyles, /@media \(max-width: 720px\)[\s\S]*\.signature-story__panel--route \.case-signal\s*\{[^}]*height:\s*31rem[^}]*min-height:\s*31rem/s, 'The authority-routing visual lacks a compact mobile height contract.')
requirePattern(workSectionStyles, /@media \(max-height: 820px\) and \(min-width: 721px\)[\s\S]*\.signature-story__panel\s*\{[^}]*position:\s*relative[^}]*top:\s*auto[^}]*\}[\s\S]*\.signature-story__panel--route \.case-signal\s*\{[^}]*height:\s*32rem[^}]*min-height:\s*32rem/s, 'Short desktop viewports keep an oversized sticky frame instead of using a readable scrolling panel.')
