...
requirePattern(page, /<Hero \/>[\s\S]*<IdentityBridge \/>[\s\S]*<WorkSection \/>[\s\S]*<EarlierSystemsSection \/>[\s\S]*<ResearchSection \/>[\s\S]*<FieldNotesSection \/>[\s\S]*<AboutSection \/>/, 'The portfolio information order changed unexpectedly.')
